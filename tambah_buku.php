...
if (isset($_POST['nama'])) {
    ...
    if ($nama == "" || $pengarang == "" || $penulis == "" || $jumlah_halaman == "" || $tanggal_upload == "" || $jenis_buku == "" || $stok == "" || $image == "") {
        echo '<script>alert("Semua data harus di isi.")</script>';
    } else {
        if (move_uploaded_file($image_tmp, $target_dir . $image)) {
            $query = mysqli_query($koneksi, "INSERT INTO buku (nama, pengarang, penulis, jumlah_halaman, tanggal_upload, jenis_buku, stok, image) VALUES ('$nama', '$pengarang', '$penulis', '$jumlah_halaman', '$tanggal_upload', '$jenis_buku', '$stok', '$image')");
            if ($query) {
